...
- AI Agent가 plan mode에서 작업 계획을 완성했을 때
...
Project
  └── Epic — Major feature unit
        └── Task — Implementation unit (task whose parent is an Epic)
              └── Task — Sub-step (task whose parent is another Task; same type, no difference)
  └── Objective — Milestone or deadline
```

Task and SubTask are identical in structure. A "SubTask" is simply a Task whose `parent_id` points to another Task instead of an Epic.
...
계획의 `[epic]`/`[task]` 구조를 다음 JSON 형식으로 변환한 뒤 한 번에 등록합니다:
      ...
      "tasks": [
        {
          ...
          "description": "optional",
          "resources": [
            {"path": "./output/report.html", "type": "output", "desc": "최종 리포트"}
          ],
          "tasks": [
            {
              "title": "Sub-step 제목 (Task의 하위 Task)",
              "description": "optional"
            }
          ]
        }
...
- `tasks` 안의 `tasks`는 서브태스크(상위 Task의 하위 Task)를 의미합니다 — 별도 타입이 아닌 동일한 구조
- `resources`와 `description`은 선택 사항
