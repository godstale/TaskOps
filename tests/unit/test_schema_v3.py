...
def test_next_workflow_id_sequential():
    conn, db_path = fresh_db()
    try:
        conn.execute(
            "INSERT INTO tasks (id, project_id, type, title, status) "
        ...
        conn.execute(
            "INSERT INTO workflows (id, project_id, title) VALUES ('TST-W001', 'TST', 'First')"
        )
        conn.commit()
        from cli.commands.utils import next_workflow_id
        wid = next_workflow_id(conn, 'TST')
