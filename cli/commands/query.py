...
def handle_generate_ops(args):
    conn = get_db(args)
    try:
        now = datetime.now().isoformat(sep=' ', timespec='seconds')
        lines = []
        lines.append("# Task Operations Log")
        lines.append("")
        lines.append(f"> Generated by TaskOps | Last updated: {now}")
        lines.append("")

        # Get all tasks that have operations
        task_ids = conn.execute(
            "SELECT DISTINCT task_id FROM operations ORDER BY task_id"
        ).fetchall()

        if not task_ids:
            lines.append("(No operations yet)")
        else:
            for tid_row in task_ids:
                tid = tid_row['task_id']
                task = conn.execute(
                ...
                lines.append("")
                lines.append("| # | Type | Platform | Time | Summary |")
                lines.append("|---|------|----------|------|---------|")
                ...
                for op in ops:
                    time = op['started_at'] or op['completed_at'] or op['created_at']
                    platform = op['agent_platform'] or '-'
                    summary = op['summary'] or '-'
                    lines.append(
                        f"| {op['id']} | {op['operation_type']} | {platform} | {time} | {summary} |"
                    )
                lines.append("")
        ...
        project_dir = get_project_dir(args)
        import os
        ops_path = os.path.join(project_dir, 'TASK_OPERATIONS.md')
        content = '\n'.join(lines)
        with open(ops_path, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"TASK_OPERATIONS.md regenerated at {ops_path}")
    finally:
        close_connection(conn)
