...
- **작업 이력 기록**: 태스크별 start/progress/complete/error/interrupt 이벤트 기록, 워크플로우 자동 연결
- **조회 및 리포트**: 프로젝트 상태, 태스크 목록, 워크플로우 상세 조회
- **리소스 연결**: 파일/URL을 태스크에 연결, 워크플로우 자동 연결
...
| `op start/progress/complete/error/interrupt/log` | 작업 이력 기록 (workflow_id 자동 설정) |
